...
├── electricity-tracker.html← Electricity usage tracker (requires login)
...
Browser (S3-hosted HTML)
        ...
        │  POST /api/auth/login
        │  GET  /api/electricity        (JWT required)
        │  POST /api/electricity        (JWT required)
        │  DELETE /api/electricity/:id  (JWT required)
        ▼
API Gateway HTTP API  (ap-south-1)  — $default catch-all route
        │
        ▼
...
login.html  ──────────────→  dashboard.html
          ...
          ┌──────────────┬─────────┴──────────┬──────────────┬────────────────────┐
          ▼              ▼                     ▼              ▼                    ▼
  calculator.html  todo-list.html  prayer-times.html  utilities.html  electricity-tracker.html
          │              │                     │              │                    │
          └──────────────┴─────────────────────┴──────────────┴────────────────────┘
                                              │
                                       ← Back to Dashboard
...
- Left sidebar with vertical project tiles (Calculator, Todo List, Prayer Times, Utilities, Electricity Tracker)
...
### Electricity Tracker (`electricity-tracker.html`)
Manual electricity usage tracker backed by DynamoDB. Requires login.

**Features:**
- Add entries with date, meter reading, usage, comments, and entry type (Regular / Bill received)
- Billing cycle tracking: usage resets to zero after each "Bill received" entry
- Cycle bar shows running usage total and last meter reading for the current cycle
- Data persisted to DynamoDB (`auth_users.electricityEntries`) via authenticated API calls
...
| `GET` | `/api/electricity` | JWT | Fetch all electricity entries for the user |
| `POST` | `/api/electricity` | JWT | Add a new electricity entry |
| `DELETE` | `/api/electricity/:id` | JWT | Delete an electricity entry by ID |
